...
public class CognitoUserService {
    ...
    public JsonObject getUserByUsername(String userName, String poolId) {
        ...
        AdminGetUserResponse adminGetUser = cognitoIdentityProviderClient.adminGetUser(adminGetUserRequest);

        JsonObject adminGetUserResponse= new JsonObject();
        adminGetUserResponse.addProperty("isSuccessful",adminGetUser.sdkHttpResponse().isSuccessful());
        adminGetUserResponse.addProperty("statusCode",adminGetUser.sdkHttpResponse().statusCode());

        List<AttributeType> userAttributes = adminGetUser.userAttributes();
        userAttributes.forEach(attributeType -> {
            adminGetUserResponse.addProperty(attributeType.name(),attributeType.value());
        });
